/*
Title: GraphWalker modelling syntax
...
The objective of the model, is to express the expected behavior of the system under test. To do so, we use a [directed graph], in which a vertex (or a node) represents some desired state, and the edges represents whatever actions we need to do in order to achieve that desired state.
...
## The Rules
This section will talk about the modeling rules. They mostly the same as for GraphWalker 2, with some exceptions.
